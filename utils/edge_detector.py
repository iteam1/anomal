'''
CMD: python3 utils/edge_detector.py --input assets/top_right_org.jpg --prototxt model/hed/deploy.prototxt --caffemodel model/hed/hed_pretrained_bsds.caffemodel
'''
import cv2 as cv
import argparse
import numpy as np
...
inp = cv.dnn.blobFromImage(image, scalefactor=2.0, size=(args.width, args.height),
...
out = cv.bitwise_and(out,edges)
out = cv.ximgproc.thinning(out)
# con=np.concatenate((edges_thinned,out_thinned),axis=1)
cv.imwrite('assets/out.jpg',out)