...
class DinkNet34(nn.Module):
    def __init__(self, num_classes=1, siis_size=[16, 16], width=1, kw=9, dim=512, arch=4, num_channels=3):
        ...
        self.siis = SIIS(self.siis_size, width, kw, dim, arch)  # size=siis_size, dim=dim
    ...
    def forward(self, x):
        ...
        e4 = self.dblock(e4)
        # SIIS
        valid_size = e4.shape[2:]
        if list(valid_size) != self.siis_size:
            # s/output_stride != siis_size(default=[32, 32])
            e4 = resize(e4, newsize=self.siis_size)
        e4 = self.siis(e4)  # fix input size: siis_size
        if list(valid_size) != e4.shape[2:]:
            # s/output_stride != siis_size(default=[32, 32])
            e4 = resize(e4, newsize=valid_size)
        # Decoder
